...
const QADetailCard = ({ qa, isDark, tickets }) => {
  const [expanded, setExpanded] = useState(false);

  // Calculate performance metrics
  const qaTickets = tickets.filter(t => t.qa_id === qa._id || t.reviewedBy === qa._id);
  const resolvedTickets = qaTickets.filter(t => t.status === 'resolved' || t.status === 'closed').length;
  const openTickets = qaTickets.filter(t => t.status === 'open').length;
  const pendingTickets = qaTickets.filter(t => t.status === 'pending').length;
  ...
  const resolutionRate = qaTickets.length > 0 
    ? Math.round((resolvedTickets / qaTickets.length) * 100)
    : 0;
...
const QAPerformanceDetail = () => {
  ...
  // Filter only QA members from all employees
  const qaTeam = useMemo(() => {
    const employees = employeesData?.data || [];
    return employees.filter(e => ['QA', 'TL'].includes(e.role));
  }, [employeesData]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-slate-900 dark:to-slate-800 p-2 md:p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Click on any QA member to view detailed performance metrics
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-2 mb-8">
        <div className="p-2 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
          <p className="text-xs text-amber-600 dark:text-amber-400 font-semibold">Total QA Members</p>
      ...
      {qaTeam.length > 0 ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700">
          <p className="text-gray-600 dark:text-gray-400">No QA team members found</p>
        </div>
      )}
    </div>
  );
